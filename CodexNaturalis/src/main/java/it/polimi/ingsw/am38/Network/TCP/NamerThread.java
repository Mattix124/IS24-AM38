...
public class NamerThread implements Runnable
{
	...
	public void run()
	{
		...
		do
		{
			try
			{
				player = initializePlayer(errorMessage, clIn, clOut);
			}
		...
		if (instruction.equals("1"))
		{
			...
			do
			{
				...
				try
				{
					lobbyManager.createNewGame(Integer.parseInt(instruction), player); //Se ritornasse il gameID ottimo
				}
				catch (NumOfPlayersException e)
				{
					errorMessage = "Your input is not valid. Retry:\n From 2 to 4 players.\n";
	...
	private Player initializePlayer(String message, Scanner clIn, PrintWriter clOut) throws NicknameTakenException, NullNicknameException
	{
		...
		return lobbyManager.createPlayer(name);
	}
}
